...
source_folder = '../../data/example_fmri'; 
destination_folder = '../../data/example_fmri_eigenvectors';
...
for i=1:numel(elements_in_dir)
    if numel(elements_in_dir(i).name)>3 && strcmp(elements_in_dir(i).name(end-2:end),'mat') % verify if it's the correct data ending in .mat
        ...
        [eigenvectors,~] = compute_eigen_timeseries(timeseries,1); % run the analytical computation of eigenvectors. VIsual output, put 0 to suppress output
