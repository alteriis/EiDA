function [position, moment] = visualize_phase_space_connectivity_leida_vtranspose(leading_eigen, centroid,video,interactive,toplot)

% here I work on matrices not eigens!
% nota ho sbagliato a cancellare, questa è quella che 
% se uso posizione = mean semplicemente 
% e momentum = mean(distanze) mi escono i cerchi

vectorized_centroid = centroid'*centroid;
vectorized_centroid = vectorized_centroid(:);
...
for i = 2:n
    pc = leading_eigen(:,i)*leading_eigen(:,i)';
    pc_im1 = leading_eigen(:,i-1)*leading_eigen(:,i-1)';
    reconf_speeds(i-1) = corr(pc(:),pc_im1(:)); % let's try just this
    position(i-1) = pc(:)'*vectorized_centroid;
    % option 1
    moment(i-1) = norm(pc)*reconf_speeds(i-1);
    % option 2
    moment(i-1) = reconf_speeds(i-1);
end
